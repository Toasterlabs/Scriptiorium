Function invoke-GetMailboxPermissions{
	Param(
		$outputPath
	)
	...
	Function Check-Delegates ([string]$DelegateID, $ExportFile) {
		...
		If ($CheckDelegate -ne $null) {
			...
			If ($CheckDelegate.RecipientType -like "Mail*" -and $CheckDelegate.RecipientType -like "*Group" -and $ExpandDistributionGroups -eq $true) {
				invoke-WriteLog "ALERT: Expand distribution group membership. [$($CheckDelegate.Name)]"
			...
			If ($CheckDelegate.RecipientType -eq "Group" -and $ExpandSecurityGroups -eq $true) {
				invoke-WriteLog "ALERT: Expand security group membership. [$($CheckDelegate.Name)]"
	...
	If ($IncludeFolderDelegates -eq $true -and $EmsVersion -eq "EX2007") {
		invoke-WriteLog "WARNING: Folder delegate permissions cannot be retrieved from Exchange 2007 and will not be exported.`n
	...
	$MailboxAccessExport = "$outputPath\MailboxAccess.csv"
	$SendAsExport = "$outputPath\MailboxSendAs.csv"
	$SendOnBehalfExport = "$outputPath\MailboxSendOnBehalf.csv"
	$FolderDelegateExport = "$outputPath\MailboxFolderDelegate.csv"
	...
	If ($UseImportFile -eq $true) {
		invoke-WriteLog "INFO: Retrieve mailboxes from user list. [$ImportFile]"
		$Mailboxes = (Import-Csv $ImportFile | Select PrimarySmtpAddress) }
	Else {
		invoke-WriteLog "INFO: Retrieving mailboxes from Exchange..."
		$Mailboxes = (Get-Mailbox -ResultSize Unlimited | Select PrimarySmtpAddress) }
	$MailboxCount = $Mailboxes.Count; $Progress = 0
	invoke-WriteLog "SUCCESS: Found $MailboxCount Mailboxes."

	# --- Process mailboxes

	ForEach ($Mailbox in $Mailboxes) {
		$progress++
		Write-Progress -Activity "Gathering Mailbox Permissions" -Status "Audit mailbox $Progress of $MailboxCount. [$MailboxEmail]"
		[string]$MailboxEmail = $Mailbox.PrimarySmtpAddress
		$CheckMailbox = Get-Recipient $MailboxEmail -ErrorAction SilentlyContinue
		If ($CheckMailbox -eq $null) { invoke-WriteLog "ERROR: Mailbox not found. [$MailboxEmail]"; Continue }
		[string]$MailboxName = $CheckMailbox.Name
		[string]$MailboxDN = $CheckMailbox.DistinguishedName

		invoke-WriteLog ""; invoke-WriteLog "INFO: Audit mailbox $Progress of $MailboxCount. [$MailboxEmail]"

		# --- Export mailbox access permissions

		If ($IncludeMailboxAccess -eq $true) {
			invoke-WriteLog "AUDIT: Mailbox access permissions..."
			$Delegates = @()
		...
		If ($IncludeSendAs -eq $true) {
			invoke-WriteLog "AUDIT: Send As permissions..."
			$Delegates = @()
		...
		If ($IncludeSendOnBehalf -eq $true) {
			invoke-WriteLog "AUDIT: Send On Behalf permissions..."
			$Delegates = @()
		...
		If ($IncludeFolderDelegates -eq $true) {
			invoke-WriteLog "AUDIT: Folder delegate permissions."
	...
	invoke-WriteLog ""; invoke-WriteLog "SUCCESS: Permission export complete!"
}